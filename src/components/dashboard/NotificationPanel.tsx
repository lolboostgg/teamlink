...
export function NotificationPanel({ notifications }: { notifications: FeedNotification[] }) {
  ...
  const visibleNotifications = notifications.filter((notification) => !notification.read || notification.actionable);
  return <div className="notification-panel">
    ...
    <div className="notification-panel__scroll">
      {!visibleNotifications.length && <div className="notification-panel__empty"><i className="fa-solid fa-check-double" />You&rsquo;re all caught up.</div>}
