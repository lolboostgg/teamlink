...
export async function GET(request: Request) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      // Belt and braces around the whole of setup. Anything thrown from
      // start() surfaces as an unhandled rejection, and Node 22 ends the
      // process on those — a single bad connection would take the server
      // down and every open tab would reconnect into the restart, which is
      // how one broken stream becomes a restart loop. One dead stream is a
      // dead stream; it is never worth the process.
      try {
